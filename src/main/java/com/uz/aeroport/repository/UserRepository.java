...
{
    @Query(value = "select id,login,password,old_Password from userss where old_Password = (:password)",nativeQuery = true)
    Optional<UserCheckEntity> findByPassword(@Param("password") String password);

    @Query(value = "select id,login,password,old_Password from userss where login = (:admin)",nativeQuery = true)
